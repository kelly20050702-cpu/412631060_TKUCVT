...
- A:基於「運維獨立性」與「緊急救援」確保app伺服器因資源耗盡或程式臭蟲（Bug）崩潰時，管理員仍可透過跳板機直接連入db進行排錯與資料保護，達成管理與業務路徑環境分離，不至於因為應用層崩潰而失去對資料庫的控制；在安全性方面，透過防火牆嚴格限制僅允許bastion的內網 IP存取 Port 22，達到安全精準且單純的效果。
